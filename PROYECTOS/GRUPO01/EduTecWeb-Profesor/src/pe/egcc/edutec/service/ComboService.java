...
public class ComboService {
  ...
  public List<ComboDTO> traerCiclosCC()
  {
	    List<ComboDTO> lista = null;
	    Connection cn = null;
	    try {
	      ...
	      // Ciclo actual
	      Calendar fechaActual = Calendar.getInstance();
	      int anio = fechaActual.get(Calendar.YEAR)+1;
	      int mes = fechaActual.get(Calendar.MONTH);
	      String ciclo = "" + anio + "-" + String.format("%02d", mes);
	      // Consulta
	      //SELECT IdCiclo, FecInicio, FecTermino FROM CICLO ORDER BY IDCICLO DESC
	      String sql = "SELECT IdCiclo, FecInicio, FecTermino FROM CICLO where idciclo >= ?";
	      PreparedStatement pstm = cn.prepareStatement(sql);
	      pstm.setString(1, ciclo);
	      ResultSet rs = pstm.executeQuery();
	      lista = rsToListCombo(rs);
	      ...
	      throw new RuntimeException("Error al acceder a la tabla de ciclos.");
	    } finally {
	      try {
	        cn.close();
	      } catch (Exception e2) {
